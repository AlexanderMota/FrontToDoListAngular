...
export class UserService {
  ...
  getInvitation(task_id:string){
    return this.http.get<UserResponse>(
        `${this.apiUrl}invitacion/${task_id}`, { withCredentials:true }
    );
  }
  getCollabsConfirmed(task_id:string){
    return this.http.get<UserResponse>(
        `${this.apiUrl}colaboradores/confirmados/${task_id}`, { withCredentials:true }
    );
  }
  getCollabsPending(task_id:string){
    return this.http.get<UserResponse>(
        `${this.apiUrl}colaboradores/pendientes/${task_id}`, { withCredentials:true }
    );
  }
  ...
  postSolicitudColaboracion(user_id:string, task_id:string) {
    ...
    return this.http.post<UserResponse>(`${this.apiUrl}colaboradores/${user_id}`,{}, {
      params: {
        task_id: task_id
      }, withCredentials: true 
    });
  ...
  updateSolicitudColaboracion(request_Id: string): Observable<UserResponse> {
    return this.http.put<UserResponse>(`${this.apiUrl}colaboradores/${request_Id}`,{},{ withCredentials: true });
  }
  
  deleteSolicitudColaboracion(request_Id: string): Observable<UserResponse> {
    return this.http.delete<UserResponse>(`${this.apiUrl}invitacion/${request_Id}`, { withCredentials: true });
  }
